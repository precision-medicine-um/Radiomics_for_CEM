    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(generator.size()):
            ...
            for d, mask in zip(detections, masks):
                ...
                mask = cv2.resize(mask, (box[2] - box[0], box[3] - box[1]),interpolation =cv2.INTER_AREA)
                ...
                if mask_image[box[1]:box[3], box[0]:box[2]].shape != mask.shape:
                    print("sizes are different")
                    mask = cv2.resize(mask, (mask_image[box[1]:box[3], box[0]:box[2]].shape[1],mask_image[box[1]:box[3], box[0]:box[2]].shape[0]),interpolation =cv2.INTER_AREA)
